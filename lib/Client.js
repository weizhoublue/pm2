...
// Inlined motd banner for fallback in packaged binaries
var MOTD_BANNER = '\n' +
'                        -------------\n' +
'\n' +
...
'     _\\/\\\\\\\\_____________\\/\\\\\\____\\\\\\///_____\\/\\\\\\____/\\\\\\//________\n' +
...
Client.prototype.initFileStructure = function (opts) {
  ...
  if (!process.env.PM2_PROGRAMMATIC && !fs.existsSync(path.join(opts.PM2_HOME, 'touch'))) {
    ...
    // Display motd banner
    // Prefer reading from file in source installations, fall back to inlined version
    try {
      var bannerPath = path.join(__dirname, opts.PM2_BANNER);
      
      if (typeof process.pkg === 'undefined') {
        // Source installation: try to read the file
        try {
          var dt = fs.readFileSync(bannerPath);
          console.log(dt.toString());
        } catch (err) {
          // File not found in source, use inlined version
          console.log(MOTD_BANNER);
        }
      } else {
        // Packaged binary: use inlined version
        console.log(MOTD_BANNER);
      }
    } catch(e) {
      debug('Failed to display motd: %s', e.message || e);
    }
