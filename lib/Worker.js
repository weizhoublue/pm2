...
module.exports = function(God) {
  ...
  var midnightRotationGeneration = 0;
  ...
  God.registerCron = function(pm2_env) {
    ...
    var job = Cron(pm2_env.cron_restart, { timezone: Timezone.get() || undefined }, function() {
  ...
  God.Worker.start = function() {
    ...
    // Schedule midnight log rotation
  ...
  God.Worker.scheduleMidnightRotation = function() {
    var generation = ++midnightRotationGeneration;

    if (midnightTimer !== null)
      clearTimeout(midnightTimer);

    function scheduleNext() {
      var delay = Timezone.msUntilNextMidnight(new Date());
      // Add a small jitter (1-30 seconds) to avoid thundering herd
      delay += Math.floor(Math.random() * 30000);

      // Safety: max delay 24h + 60s
      if (delay > 86460000) delay = 86460000;

      console.log('[PM2][WORKER] Next log rotation scheduled in %d ms (at midnight)', delay);

      midnightTimer = setTimeout(function() {
        if (generation !== midnightRotationGeneration)
          return;

        God.rotateDateLogs(function(err) {
          if (err) console.error('[PM2][WORKER] Log rotation error:', err);
          if (generation !== midnightRotationGeneration)
            return;
          scheduleNext();
        });
