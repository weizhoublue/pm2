...
function exec(script, stds) {
  process.on('message', function (msg) {
    ...
    if (msg.type === 'log:reload') {
      // If new paths provided (midnight date rotation), update local paths
      if (msg.data) {
        if (msg.data.pm_out_log_path) outFile = msg.data.pm_out_log_path;
        if (msg.data.pm_err_log_path) errFile = msg.data.pm_err_log_path;
        if (msg.data.pm_log_path) stdFile = msg.data.pm_log_path;
      }
      ...
      // Rebuild stds with (potentially new) paths
      stds.out = outFile;
      stds.err = errFile;
      stdFile && (stds.std = stdFile);
      ...
      console[method] = (...args) => {
        ...
        if (pm2_env.log_type && pm2_env.log_type === 'json') {
          log_data = JSON.stringify({
            message : msg,
            timestamp : Timezone.format(new Date(), pm2_env.log_date_format || 'YYYY-MM-DDTHH:mm:ssZ', pm2_env.pm2_timezone),
            type : 'err',
          ...
          log_data = `${Timezone.format(new Date(), pm2_env.log_date_format, pm2_env.pm2_timezone)}: ${msg}`;
        else
    ...
    process.stderr.write = (function(write) {
      return function(string, encoding, cb) {
        ...
        if (pm2_env.log_type && pm2_env.log_type === 'json') {
          ...
          log_data = `${Timezone.format(new Date(), pm2_env.log_date_format, pm2_env.pm2_timezone)}: ${string.toString()}`;
        else
