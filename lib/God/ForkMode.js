    ...
    Utility.startLogging(stds, function(err, result) {
      ...
      function prefixLogWithDate(pm2_env, data) {
        ...
        if (log_data.length > 1)
          log_data.pop()
        log_data = log_data.map(line => `${Timezone.format(new Date(), pm2_env.log_date_format, pm2_env.pm2_timezone)}: ${line}\n`)
      ...
      cspr._reloadLogs = function(cb) {
        ...
        // Refresh paths from pm2_env for date rotation (midnight path update)
        if (cspr.pm2_env) {
          stds.out = cspr.pm2_env.pm_out_log_path || stds.out;
          stds.err = cspr.pm2_env.pm_err_log_path || stds.err;
          if (cspr.pm2_env.pm_log_path) {
            stds.std = cspr.pm2_env.pm_log_path;
          }
        }
