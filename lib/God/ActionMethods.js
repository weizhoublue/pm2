...
module.exports = function(God) {
  God.refreshTimezone = function refreshTimezone(env, cb) {
    var timezone = Timezone.get();

    God.CronJobs.forEach(function(job) {
      job.stop();
    });
    God.CronJobs.clear();

    Object.keys(God.clusters_db).forEach(function(id) {
      var cluster = God.clusters_db[id];
      if (cluster && cluster.pm2_env) {
        cluster.pm2_env.pm2_timezone = timezone;
        God.registerCron(cluster.pm2_env);
        if (cluster.send) {
          try {
            cluster.send({ type: 'timezone:update', data: { timezone: timezone } });
          } catch (err) {
    ...
    return cb(null, { timezone: timezone, cronJobs: God.CronJobs.size });
  };
  ...
  God.resetMetaProcessId = function(id, cb) {
    ...
    God.resetRuntimeStats(God.clusters_db[id].pm2_env);
        ...
        if (cluster.send && cluster.pm2_env.exec_mode == 'cluster_mode') {
          try {
            cluster.send({
              type:'log:reload',
              data: {
                pm_out_log_path: cluster.pm2_env.pm_out_log_path,
                pm_err_log_path: cluster.pm2_env.pm_err_log_path,
                pm_log_path: cluster.pm2_env.pm_log_path
              }
            });
          } catch(e) {
  ...
  /**
   * Rotate date-stamped log files at midnight.
   * Computes new date-suffixed paths for all processes with log_date_files enabled,
   * updates pm2_env, and reloads all log streams.
   * Also triggers cleanup of old log files.
   * @method rotateDateLogs
   * @param {} cb
   */
  God.rotateDateLogs = function(cb) {
    var dateStr = Timezone.dateKey(new Date());
    var processIds = Object.keys(God.clusters_db);

    processIds.forEach(function (id) {
      var cluster = God.clusters_db[id];
      if (!cluster || !cluster.pm2_env) return;
      var env = cluster.pm2_env;

      if (env._dateLogUseDefaults) {
        var formated_name = env.name.replace(/[^a-zA-Z0-9\\.\\-]/g, '-');
        var mergedPath = Common.makeDateLogPath(formated_name, dateStr);

        env.pm_out_log_path = mergedPath;
        env.pm_err_log_path = mergedPath;
        delete env.pm_log_path;
      }
    });

    God.reloadLogs({}, function(err) {
      if (err) console.error('[PM2] Error reloading logs during rotation:', err);
      God.cleanupOldDateLogs(function(cleanupErr) {
        if (cleanupErr) console.error('[PM2] Error cleaning up old logs:', cleanupErr);
  ...
  /**
   * Delete date-stamped log files older than log_date_retain_days.
   * Only cleans files in the default log directory matching the pattern
   * <name>-<YYYYMMDD>.log or <name>-<pm_id>-<YYYYMMDD>.log
   * @method cleanupOldDateLogs
   * @param {} cb
   */
  God.cleanupOldDateLogs = function(cb) {
    var fs = require('fs');
    var path = require('path');
    var dayjs = require('dayjs');

    var logDir = cst.DEFAULT_LOG_PATH;
    var retainDays = cst.LOG_DATE_RETAIN_DAYS;

    if (!retainDays || retainDays < 1) retainDays = 30;

    // Check global config for override
    var conf = require('../Configuration.js');
    var globalConfig = conf.getSync('pm2');
    if (globalConfig && globalConfig.log_date_retain_days) {
      retainDays = parseInt(globalConfig.log_date_retain_days) || retainDays;
    }

    var cutoff = dayjs().subtract(retainDays, 'day');

    fs.readdir(logDir, function(err, files) {
      if (err) {
        if (err.code === 'ENOENT') return cb ? cb(null) : null;
        return cb ? cb(err) : null;
      }

      var dateFileRegex = /^(.+)-(\d{8})\.log$/;
      var deletedCount = 0;

      files.forEach(function(file) {
        var match = file.match(dateFileRegex);
        if (!match) return;

        var fileDateStr = match[2];
        var fileDate = dayjs(fileDateStr, 'YYYYMMDD');
        ...
        if (fileDate.isBefore(cutoff)) {
          ...
          try {
            ...
            deletedCount++;
            console.log('[PM2] Cleaned up old log file: ' + file);
          } catch(e) {
            console.error('[PM2] Failed to delete old log file: ' + file, e.message);
          }
        }
      });

      if (deletedCount > 0) {
        console.log('[PM2] Cleaned up ' + deletedCount + ' old log file(s) older than ' + retainDays + ' days');
      }
