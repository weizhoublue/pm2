...
God.init = function() {
  ...
  // Clean up old date-stamped log files on startup
  setTimeout(function() {
    God.cleanupOldDateLogs(function(err) {
      if (err) console.error('[PM2] Startup log cleanup error:', err);
    });
  }, 10000);
}
...
God.ensureRuntimeStats = function(pm2_env) {
  if (!pm2_env)
    return pm2_env

  if (typeof pm2_env.start_count !== 'number')
    pm2_env.start_count = 0
  if (typeof pm2_env.success_count !== 'number')
    pm2_env.success_count = 0
  if (typeof pm2_env.failure_count !== 'number')
    pm2_env.failure_count = 0
  if (typeof pm2_env.last_exit_code === 'undefined')
    pm2_env.last_exit_code = null
  if (typeof pm2_env.last_exit_at === 'undefined')
    pm2_env.last_exit_at = null

  return pm2_env
}

God.resetRuntimeStats = function(pm2_env) {
  if (!pm2_env)
    return pm2_env

  God.ensureRuntimeStats(pm2_env)
  ...
  pm2_env.last_exit_code = null
  pm2_env.last_exit_at = null
  pm2_env.exit_code = null

  return pm2_env
}

God.resolveStatsTarget = function(pm2_env) {
  if (!pm2_env)
    return null

  var pm_id = pm2_env.pm_id

  if (typeof pm_id === 'string' && pm_id.indexOf('_old_') === 0) {
    var real_id = pm_id.replace('_old_', '')
    var current = God.clusters_db[real_id]

    if (current && current.pm2_env)
      return current.pm2_env
  }

  return pm2_env
}

God.recordExitStats = function(pm2_env, exit_code) {
  var target = God.resolveStatsTarget(pm2_env)

  if (!target)
    return

  God.ensureRuntimeStats(target)

  target.last_exit_code = typeof exit_code === 'undefined' ? null : exit_code
  target.last_exit_at = Date.now()

  if (exit_code === 0)
    target.success_count += 1
  else if (typeof exit_code !== 'undefined')
...
God.executeApp = function executeApp(env, cb) {
  ...
  God.ensureRuntimeStats(env_copy)
  ...
  if (env_copy['pm_id'] === undefined) {
    ...
    // If date-based log files are enabled, compute paths with pm_id directly
    // (skip regex-based injection which would mangle the date portion)
    if (env_copy._dateLogUseDefaults) {
      var dayjs = require('dayjs');
      var dateStr = dayjs().format('YYYYMMDD');
      var formated_name = env_copy.name.replace(/[^a-zA-Z0-9\\.\\-]/g, '-');
      var mergedPath = Common.makeDateLogPath(formated_name, env_copy.pm_id, dateStr);
      env_copy.pm_out_log_path = mergedPath;
      env_copy.pm_err_log_path = mergedPath;
      env_copy.pm_log_path = mergedPath;
    } else if (!env_copy['merge_logs']) {
...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  ...
  God.recordExitStats(proc.pm2_env, exit_code)
