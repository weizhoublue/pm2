...
Common.prepareAppConf = function(opts, app) {
  ...
  // Check custom file paths BEFORE the loop deletes them from the app
  var hasCustomOutFile = !!app.out_file;
  var hasCustomErrorFile = !!app.error_file;
  var hasCustomLogFile = !!app.log_file;
  ...
  // If log_date_files is enabled and user didn't set custom log paths,
  // merge stdout and stderr into a single date-stamped file
  var logDateFilesEnabled = (app.log_date_files !== false); // default true

  if (logDateFilesEnabled && !hasCustomOutFile && !hasCustomErrorFile && !hasCustomLogFile) {
    var dayjs = require('dayjs');
    var dateStr = dayjs().format('YYYYMMDD');
    var paths = Common.getDateLogPaths(formated_app_name, dateStr);

    var logDir = path.dirname(paths.pm_out_log_path);
    if (!fs.existsSync(logDir)) {
      try { require('mkdirp').sync(logDir); } catch(e) {}
    }

    app.pm_out_log_path = paths.pm_out_log_path;
    app.pm_err_log_path = paths.pm_err_log_path;
    delete app.pm_log_path;
  }

  app._dateLogUseDefaults = logDateFilesEnabled && !hasCustomOutFile && !hasCustomErrorFile && !hasCustomLogFile;

  return app;
};

/**
 * Compute a date-stamped log file path.
 * @method makeDateLogPath
 * @param {String} baseName  sanitized app name
 * @param {Number} pmId      optional process id for cluster instances
 * @param {String} dateStr   date string in YYYYMMDD format
 * @return {String} resolved absolute path
 */
Common.makeDateLogPath = function(baseName, dateStr) {
  return path.resolve(cst.DEFAULT_LOG_PATH, baseName + '-' + dateStr + '.log');
};

/**
 * Get date-stamped log paths for a given app name.
 * @method getDateLogPaths
 * @param {String} appName  original app name (will be sanitized)
 * @param {String} dateStr  date string in YYYYMMDD format
 * @return {Object} { pm_out_log_path, pm_err_log_path, pm_log_path: undefined }
 */
Common.getDateLogPaths = function(appName, dateStr) {
  var formated = appName.replace(/[^a-zA-Z0-9.\-]/g, '-');
  var mergedPath = Common.makeDateLogPath(formated, dateStr);
  return {
    pm_out_log_path: mergedPath,
    pm_err_log_path: mergedPath,
    pm_log_path: undefined
  };
};
...
Common.safeExtend = function(origin, add){
  ...
  var keysToIgnore = ['name', 'exec_mode', 'env', 'args', 'pm_cwd', 'exec_interpreter', 'pm_exec_path', 'node_args', 'pm_out_log_path', 'pm_err_log_path', 'pm_pid_path', 'pm_id', 'status', 'pm_uptime', 'created_at', 'windowsHide', 'username', 'merge_logs', 'kill_retry_time', 'prev_restart_delay', 'instance_var', 'unstable_restarts', 'restart_time', 'axm_actions', 'pmx_module', 'command', 'watch', 'filter_env', 'versioning', 'vizion_runing', 'MODULE_DEBUG', 'pmx', 'axm_options', 'created_at', 'watch', 'vizion', 'axm_dynamic', 'axm_monitor', 'instances', 'automation', 'autostart', 'autorestart', 'stop_exit_codes', 'unstable_restart', 'treekill', 'exit_code', 'last_exit_code', 'last_exit_at', 'start_count', 'success_count', 'failure_count', 'vizion', 'log_date_files', 'log_date_retain_days'];
