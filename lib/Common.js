...
Common.prepareAppConf = function(opts, app) {
  ...
  if (logDateFilesEnabled && !hasCustomOutFile && !hasCustomErrorFile) {
    var dayjs = require('dayjs');
    var dateStr = dayjs().format('YYYYMMDD');
    var mergedPath = Common.makeDateLogPath(formated_app_name, undefined, dateStr);
    ...
    app.pm_log_path = mergedPath;
  }

  app._dateLogUseDefaults = logDateFilesEnabled && !hasCustomOutFile && !hasCustomErrorFile;
 ...
 */
Common.makeDateLogPath = function(baseName, pmId, dateStr) {
  var fileName = baseName + (pmId !== undefined ? '-' + pmId : '') + '-' + dateStr + '.log';
  return path.resolve(cst.DEFAULT_LOG_PATH, fileName);
};
