...
module.exports = function(CLI) {
  ...
  CLI.prototype.set = function(key, value, cb) {
    ...
    Configuration.set(key, value, function(err) {
      ...
      if (key === 'pm2:timezone') {
        return that.Client.executeRemote('refreshTimezone', {}, function(err) {
          if (err)
            return cb ? cb(Common.retErr(err)) : that.exitCli(cst.ERROR_EXIT);
          return cb ? cb(null, {success:true}) : that.exitCli(cst.SUCCESS_EXIT);
        });
  ...
  CLI.prototype.unset = function(key, cb) {
    ...
    Configuration.unset(key, function(err) {
      ...
      if (key === 'pm2:timezone') {
        return that.Client.executeRemote('refreshTimezone', {}, function(refreshErr) {
          if (refreshErr)
            return cb ? cb(Common.retErr(refreshErr)) : that.exitCli(cst.ERROR_EXIT);
          return displayConf(function() { cb ? cb(null, {success:true}) : that.exitCli(cst.SUCCESS_EXIT) });
        });
      }
