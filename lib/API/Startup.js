...
var initScriptTemplates = require('../templates/init-scripts.js');
...
function resolvePm2BinPath() {
  var pm2_bin_path = require.main && require.main.filename;
  var local_pm2_bin_path = path.resolve(__dirname, '../../bin/pm2');

  if (typeof process.pkg !== 'undefined' ||
      (typeof pm2_bin_path === 'string' && pm2_bin_path.indexOf('/snapshot/') === 0)) {
    return process.execPath;
  }

  if (fs.existsSync(local_pm2_bin_path) === true)
    return local_pm2_bin_path;
  if (typeof pm2_bin_path === 'string' &&
      pm2_bin_path.includes('/lib/binaries/CLI.js') === true) {
    return pm2_bin_path.replace('/lib/binaries/CLI.js', '/bin/pm2');
  }

  return 'pm2';
}

function getStartupTemplate(type) {
  var templatePath = path.join(__dirname, '..', 'templates/init-scripts', type + '.tpl');
  var hasInlinedTemplate = Object.prototype.hasOwnProperty.call(initScriptTemplates, type);

  if (typeof process.pkg !== 'undefined') {
    if (hasInlinedTemplate)
      return initScriptTemplates[type];
...
function requiresLaunchdUser(uid, platform, user, home) {
  return uid === 0 &&
    ['macos', 'darwin', 'launchd'].indexOf(platform) !== -1 &&
    (!user || !home);
}
...
module.exports = function(CLI) {
  ...
  function isNotRoot(startup_mode, platform, opts, cb) {
    ...
    var pm2_bin_path = resolvePm2BinPath();

    if (opts.user) {
      console.log('sudo env PATH=$PATH:' + path.dirname(process.execPath) + ' ' + pm2_bin_path + ' ' + opts.args[1].name() + ' ' + platform + ' -u ' + opts.user + ' --hp ' + process.env.HOME);
  ...
  CLI.prototype.startup = function(platform, opts, cb) {
    ...
    if (requiresLaunchdUser(process.getuid(), platform, opts.user, opts.hp)) {
      var error = new Error('launchd startup run as root requires --user and --hp');
      Common.printError(cst.PREFIX_MSG_ERR + error.message);
      Common.printOut('sudo env PATH="$PATH" pm2 startup launchd -u <username> --hp <home>');
      return cb(error);
    }
    ...
    case 'systemd':
      if (opts.waitIp)
        template = getStartupTemplate('systemd-online');
      else
        template = getStartupTemplate('systemd');
    ...
    case 'ubuntu12':
    case 'upstart':
      template = getStartupTemplate('upstart');
    ...
    case 'darwin':
    case 'launchd':
      template = getStartupTemplate('launchd');
    ...
    case 'freebsd':
    case 'rcd':
      template = getStartupTemplate('rcd');
      service_name = (opts.serviceName || 'pm2_' + user);
    ...
    case 'openbsd':
    case 'rcd-openbsd':
      template = getStartupTemplate('rcd-openbsd');
      service_name = (opts.serviceName || 'pm2_' + user);
      ...
      break;
    case 'openrc':
      template = getStartupTemplate('openrc');
      service_name = openrc_service_name;
      destination = '/etc/init.d/' + service_name;
      commands = [
        'chmod +x ' + destination,
    ...
    case 'sunos':
    case 'solaris':
      template = getStartupTemplate('smf');
      service_name = (opts.serviceName || 'pm2_' + user);
   ...
   * Disabled by default; enable explicitly with pm2 set pm2:autodump true
   */
  CLI.prototype.autodump = function(cb) {
    if (this.pm2_configuration.autodump !== true && this.pm2_configuration.autodump !== 'true') {
      return cb();
    }
...
module.exports.getStartupTemplate = getStartupTemplate;
module.exports.resolvePm2BinPath = resolvePm2BinPath;
module.exports.requiresLaunchdUser = requiresLaunchdUser;
