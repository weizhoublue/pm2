...
function renderCronRestart(cronRestart) {
  if (!cronRestart || cronRestart === '0')
    return chalk.gray('-')
...
function renderExitCode(exitCode) {
  if (exitCode === null || typeof exitCode === 'undefined')
    return chalk.gray('-')

  if (exitCode === 0)
    return chalk.green('0')

  return chalk.red(String(exitCode))
}

function renderExitTime(lastExitAt) {
  if (!lastExitAt)
    return chalk.gray('-')

  return Timezone.format(lastExitAt, 'YYYY-MM-DD HH:mm:ss')
}
...
function listModulesAndAppsManaged(list, commander) {
  ...
  var sortField = 'name', sortOrder = 'asc', sort,
      fields = {
        ...
        exit_code: 'pm2_env.last_exit_code',
        exit_at: 'pm2_env.last_exit_at',
        starts: 'pm2_env.start_count',
        ok: 'pm2_env.success_count',
        fail: 'pm2_env.failure_count'
      }
  ...
  list.forEach(function(l) {
    ...
    if (l.pm2_env.pmx_module == true) {
      ...
      // Cron restart
      obj[key].push(renderCronRestart(l.pm2_env.cron_restart))

      // Last exit code
      obj[key].push(renderExitCode(
        l.pm2_env.last_exit_code !== undefined ? l.pm2_env.last_exit_code : l.pm2_env.exit_code
      ))

      // Last exit time
      obj[key].push(renderExitTime(l.pm2_env.last_exit_at))

      // Starts / success / failure
      obj[key].push(l.pm2_env.start_count || 0)
      obj[key].push(l.pm2_env.success_count || 0)
      obj[key].push(l.pm2_env.failure_count || 0)
