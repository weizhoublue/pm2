...
# Usage: run_test <test_name> <cmd> [args...]
run_test() {
  local test_name="$1"
  shift
...
run_test "pm2 start echo test" "$PM2_BIN" start bash --name smoke-test-1 -- -c 'echo hello'
...
if [ "$(id -u)" -eq 0 ]; then
  run_test "pm2 startup" "$PM2_BIN" startup
else
  echo "Skipping: pm2 startup (requires root privileges)"
fi

# Test 11: pm2 save (save process list)
run_test "pm2 save" "$PM2_BIN" save

# Test 12: pm2 kill (stop daemon)
run_test "pm2 kill" "$PM2_BIN" kill

# Test 13: pm2 list (should be empty after kill)
